...
from util.data_manager import (add_kid_data, add_overdue_assignment,
                               calculate_kid, concat_overdue_assignments,
                               get_subreport_by_position, pack_to_json,
                               recalculate_curators_data, remove_not_executors,
                               remove_uuid)
...
def get_data(request):
    ...
    app_name = 'server'
    report_name = 'kid_full'

    executors_data = execute_sql(app_name, 'kid', parameters)
    pns_data = execute_sql('debug', 'd_overdue_assignments', parameters)

    kid_data = calculate_kid(executors_data, pns_data)
    executors_data = add_kid_data(executors_data, kid_data, True, True)
    executors_data = add_overdue_assignment(executors_data, pns_data)
    ...
    executors_data = concat_overdue_assignments(executors_data)
